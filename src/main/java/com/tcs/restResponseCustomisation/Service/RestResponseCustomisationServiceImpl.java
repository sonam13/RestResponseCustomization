...
@Service
public class RestResponseCustomisationServiceImpl implements RestResponseCustomisationService
{
	public void generateCustomResponse(Request request) throws JSONException/*, JsonGenerationException, JsonMappingException,*/,IOException{
		//String xmlString=request.getServiceResponse().toString();
		ObjectMapper mapper = new ObjectMapper();
		mapper.writeValue(new File(".\\staff.json"), request.getServiceResponse());
		String jsonInString = mapper.writeValueAsString(request.getServiceResponse());
		JSONObject json = new JSONObject(jsonInString);
		 ...
		 String xml = XML.toString(json);
				...
				String xml1=xml.replaceFirst("<", "<root><");
				 ...
				 String str = null;
				if( ind>=0 )
				      str = new StringBuilder(xml2).replace(ind, ind+1,">\n</root>").toString();
				 System.out.println(str);
				 System.out.println("h66i");
				 System.out.println("Xml Input:" + str);
			...
			private static void getNodeDetails(NodeList childNodes, String rootElement,Request request, List list) {
				...
				classFormat.append("@XmlRootElement(name=\"" + request.getServiceOperationName() + "\")\n");
				classFormat.append("public class " + request.getServiceOperationName() + " {\n");
